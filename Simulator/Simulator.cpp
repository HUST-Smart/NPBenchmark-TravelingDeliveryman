...
namespace szx {
	...
	void Simulator::generateInstance(const InstanceTrait &trait) {//算例生成
		...
		//input.mutable_airport()->set_bridgenum(rand.pick(trait.bridgeNum.begin, trait.bridgeNum.end));
		 ...
		 /*
		   1.将边分五批构造，且边的初始节点大于终止节点：
		   1.第一批确保有解,构成一个闭合回路,第二批确保回到起点的边有过条
		   3.第三批边数所占比例为第一批所剩的1/2，第四批所占比例为第一批所剩的3/8，第五批所占比例为第一批所剩的1/8
		 */
		...
		for (int i = 0; i != edgeNum; ++i) {
			...
			if (nodeNum < 20) {//构造小算例
				if (i < nodeNum) {
					...
					while (target == source || target == source - 1) {
			...
			{
				int temp1 = (int)(nodeNum / 3);
				int temp2 = nodeNum + (int)(nodeNum * 0.2);
				int temp3 = edgeNum - temp2;
				if (i < nodeNum) {
					edge.set_source(i);
					if (i != nodeNum - 1)
						edge.set_target(i + 1);
				...
				else if (i <= temp2 + (int)(temp3*0.5)) {

					int source = rand.pick(0, temp1);
					edge.set_source(source);
					int target = rand.pick(source + 2, nodeNum);
					edge.set_target(target);

				}
				else if (i <= temp2 + (int)(temp3*0.5) + (int)(temp3*0.375)) {
					int source = rand.pick(temp1 + 1, temp1 * 2);
					edge.set_source(source);
					int target = rand.pick(source + 2, nodeNum);
					edge.set_target(target);
					...
					int source = rand.pick(temp1 * 2 + 1, nodeNum - 2);
					edge.set_source(source);
					int target = rand.pick(source + 2, nodeNum);
					edge.set_target(target);
